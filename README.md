...
[cite_start]本系統為輕量化 Web 線上圖書借閱系統，專注於後端關聯式資料庫的 CRUD 邏輯實作，不刻意追求華麗視覺 。主要技術特色如下：
* [cite_start]**核心功能**：具備即時庫存控管與動態借還書紀錄 。
* [cite_start]**權限分流**：區分「一般使用者(學生)」與「管理者」兩種角色 。
* [cite_start]**資訊安全**：導入 Session 驗證防護與參數化查詢（防範 SQL 注入） 。
* [cite_start]**實務應用**：確保系統在學校環境下，具備資料庫快速備份與還原的穩定性 。
